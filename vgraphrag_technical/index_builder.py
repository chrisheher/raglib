"""
index_builder.py (technical)
=============================
Stage 2: Technical RKG -> Three Indexes in ChromaDB

Same three-index shape as vgraphrag/index_builder.py (Node / Relationship /
Community), pointed at a separate ChromaDB path and collection names so the
technical graph never collides with the literary one. Embedding (BGE-M3)
and Leiden community detection are reused directly from vgraphrag/ — they
are fully generic graph/text operations with no domain-specific behavior.

Indexes are stored in vgraphrag_technical_db/chroma/.
"""
...
from vgraphrag.index_builder import embed_texts, detect_leiden_communities
...
# ─── collections ──────────────────────────────────────────────
NODE_COLLECTION         = "vgraphrag_tech_nodes"
RELATIONSHIP_COLLECTION = "vgraphrag_tech_relationships"
COMMUNITY_COLLECTION    = "vgraphrag_tech_communities"
...
def _generate_community_summary(
    ...
    try:
        ...
        return (
            ...
            f"Documents: {', '.join(community['works'][:10])}."
        )
...
def build_all_indexes(graph_path: Path = GRAPH_PATH, force: bool = False) -> dict:
    print(f"\nLoading technical RKG from {graph_path}...")
